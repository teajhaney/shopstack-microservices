...
const target = require('./apps/gateway/src/products/products.controller');
...
if (detected) {
    ...
    if (typeof detected.listen === 'function' && detected.mountpath === '/') { 
         ...
         detected.listen(3889, () => console.log('Server started on 3889'));
    } else {
        // Assume Router
        console.log('Detected Router or Middleware, mounting to / ...');
        app.use('/', detected);
        app.listen(3889, () => console.log('Server started on 3889'));
    }
} else {
